...
public:
	...
	sc::result react(const __evEntryAct &)
	{
		...
		if (lf_push_pixels >= push2_stage1_min_pixels) {
			...
			FS_DEV.m_motorLZ->start_by_ss(FS_SPEC.push2_stage1_speed, m_lm_steps);
		}
		if (rf_push_pixels >= push2_stage1_min_pixels) {
			m_rm_steps = FS_SPEC.yz_pixel_to_rz_step(rf_push_pixels);
			FS_DEV.m_motorRZ->start_by_ss(FS_SPEC.push2_stage1_speed, m_rm_steps);
		}
	...
	sc::result react(const evMotorLZReady & evt)
	{
		...
		const double error = (lz_nm_per_step - FS_SPEC.lz_nm_per_step) / FS_SPEC.lz_nm_per_step;
		if (error < 0.30) {
	...
	sc::result react(const evMotorRZReady & evt)
	{
		...
		const double error = (rz_nm_per_step - FS_SPEC.rz_nm_per_step) / FS_SPEC.rz_nm_per_step;
		if (error < 0.30) {
...
public:
	...
	sc::result react(const __evEntryAct &)
	{
		...
		if (!m_lf_done) {
			FS_DEV.m_motorLZ->start_by_speed(FS_SPEC.z_speed_from_pixel(m_lf_push_pixels), motor::go_forward);
		}

		if (!m_rf_done) {
			FS_DEV.m_motorRZ->start_by_speed(FS_SPEC.z_speed_from_pixel(m_rf_push_pixels), motor::go_forward);
		}
	...
	sc::result react(const evMotorLZReady & evt)
	{
		...
		if (!FS_SPEC.is_z_dist_ok(m_lf_push_pixels)) {
			log_warning("%s: left cross over or not reach dest (%3.1f pixel)", FS_STATE_NAME, m_lf_push_pixels);
			FS_DAT.code = fs_err_t::fiber_cross_over;
			return transit<stWaitReset>();
		}
	...
	sc::result react(const evMotorRZReady & evt)
	{
		...
		if (!FS_SPEC.is_z_dist_ok(m_rf_push_pixels)) {
			log_warning("%s: right cross over or not reach dest (%3.1f pixel)", FS_STATE_NAME, m_rf_push_pixels);
			FS_DAT.code = fs_err_t::fiber_cross_over;
			return transit<stWaitReset>();
		}
