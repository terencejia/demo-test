...
sc::result stPush1_initial::react(const __evCheckFbrIn & evt)
{
	const gray_img & img = evt.img.yz_img;
	m_rf_in = img_process::is_rf_in(img, FS_IPP);
	m_lf_in = img_process::is_lf_in(img, FS_IPP);
...
sc::result stPush1_initial::react(const __evFbrAllIn &)
{
	return transit<stPush1_stage1>();
}

sc::result stPush1_initial::react(const evMotorStop<motorId_t::LZ> &)
{
	if (!m_lf_in) {
...
sc::result stPush1_initial::react(const evMotorStop<motorId_t::RZ> &)
{
	if (!m_rf_in) {
		FS_DAT.code = fs_err_t::no_fiber;
		return transit<stWaitReset>();
...
private:
	...
	{
		///\todo  delete  existence of x/y motor
		if (FS_DEV.m_motorX && FS_DEV.m_motorY) {
			return transit<stPrepareCalibrating>();
		} else {
