...
, m_y_stopped(false)
{
	log_debug("motor test: prepare precise calibrating");
...
sc::result stPreparePreciseCalibrating::react(const evMotorStop<motorId_t::X> &)
{
	if (m_y_stopped) {
		if (!m_reverse_start) {
			FS_AUX.m_mt_tmr.stop();
...
sc::result stPreparePreciseCalibrating::react(const evMotorStop<motorId_t::Y> &)
{
	if (m_x_stopped) {
		if (!m_reverse_start) {
			FS_AUX.m_mt_tmr.stop();
...
sc::result stPreparePreciseCalibrating::react(const evMotorTestTimeout &)
{
