...
{
	FS_DEV.m_display.set_zoom({0, static_cast<uint16_t>(FS_HWINFO.cmos_win_height/4), static_cast<uint16_t>(FS_HWINFO.cmos_win_width), static_cast<uint16_t>(FS_HWINFO.cmos_win_height/2),
				0, static_cast<uint16_t>(FS_HWINFO.cmos_win_height/4), static_cast<uint16_t>(FS_HWINFO.cmos_win_width), static_cast<uint16_t>(FS_HWINFO.cmos_win_height/2)});
	context<stRunning>().m_stamps.push_back(std::make_tuple(FS_STATE_NAME, exemodel::monotonic_timeree::info::get_time()));
}
	...
	try {
		///detect defect
		{
			...
			if (FS_DAT.defect_data.check(ifd_msk)) {
				...
				/// adjust cmos x window horizontal coordinate
				/// by vertex pos difference between x and y img, x follow y
				{
				...
				/**
				*  adjust cmos window vertical position
