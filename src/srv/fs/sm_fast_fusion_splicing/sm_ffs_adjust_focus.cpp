...
struct stAF
: sc::state< stAF, stFS_stage1::orthogonal< 3 >,
...
public:
	...
	, data{ {
			...
			[this](void) -> void { FS_SM.active_delay_waiter< evFocusImg<cmosId_t::X> >(); },
			FS_DEV.m_x_focus ? FS_CFG.XImageFocus : false,
			true,
			0,
			FS_HWINFO.x_liquid_lens_max_val,
			16,
			FS_DEV.m_x_focus ? static_cast<int>(FS_HWINFO.x_liquid_lens_max_val * FS_DEV.m_x_focus->read()) : 0,
			...
			[this](void) -> void { FS_SM.active_delay_waiter< evFocusImg<cmosId_t::Y> >(); },
			FS_DEV.m_y_focus ? FS_CFG.YImageFocus : false,
			false,
			0,
			FS_HWINFO.y_liquid_lens_max_val,
			16,
			FS_DEV.m_y_focus ? static_cast<int>(FS_HWINFO.y_liquid_lens_max_val * FS_DEV.m_y_focus->read()) : 0,
			decltype(af_data_t::search_list)(),
			0,
			FS_HWINFO.y_liquid_lens_max_val,
		}, }
...
private:
	sc::result __check_all_done(void)
	{
		    ...
		    && state_downcast< const stAF_done<cmosId_t::Y> * >() != 0)
		{
			...
			return transit<stAdjustFocusDone>();
		}
		else {
...
stAdjustFocusDone::stAdjustFocusDone(my_context ctx)
: my_base(ctx)
{
	post_event(evAdjustFocusDone());
}

stAdjustFocusDone::~stAdjustFocusDone()
{
}
