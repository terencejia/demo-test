...
sc::result stDefectDetecting::react(const evHvbReady & evt)
{
	...
	try {
		/// \note update x/y spliter in spec
		FS_SPEC.x_spliter = img_process::get_spliter(m_img.xz_img, FS_IPP);
		FS_SPEC.y_spliter = img_process::get_spliter(m_img.yz_img, FS_IPP);

		img_process::detect_defect(
			m_img, FS_IPP,
			{
				FS_DAT.motor_tested_times <= 0 ? FS_CFG.CutAngleLimit : (FS_CFG.CutAngleLimit * 2),
				FS_DAT.motor_tested_times <= 0 ? FS_CFG.FiberAngleLimit : (FS_CFG.FiberAngleLimit * 2),
				true,
			},
			FS_DAT.defect_data);
		/// \todo delete me, only for test
		img_process::print_defects(FS_DAT.defect_data);
		img_process::save_pgm("defect_detect_yz.pgm", m_img.xz_img);
		img_process::save_pgm("defect_detect_xz.pgm", m_img.yz_img);
