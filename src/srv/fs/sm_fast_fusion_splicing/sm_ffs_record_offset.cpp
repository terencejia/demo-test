...
sc::result stRecordOffset::react(const __evImgReady & evt)
{
	...
	FS_DAT.z_record_off_set.core_diff_pre = FS_SPEC.yz_pixel_to_nm(core_diff_pixel) / 1000.0;
	...
	FS_DAT.z_record_off_set.cladding_diff_pre = FS_SPEC.yz_pixel_to_nm(cladding_diff_pixel) / 1000.0;

	FS_DAT.z_record_off_set.vertex_intersect_angle = img_process::vertex_axial_angle(evt.img, FS_IPP);

	DCL_ZMSG(record_off_set)& msg = FS_DAT.z_record_off_set;
	FS_REPORT(msg);

	return transit<stFS_Discharge>();
}
