...
sc::result stDefectDetecting::react(const evHvbReady & evt)
{
	...
	try {
		...
		/// \note store abstract for following shrink detect (avoid can't identify fiber problem)
		FS_DAT.x_left_abs = img_process::get_left_abstract(m_img.xz_img, FS_IPP);
		FS_DAT.x_right_abs = img_process::get_right_abstract(m_img.xz_img, FS_IPP);
		FS_DAT.y_left_abs = img_process::get_left_abstract(m_img.yz_img, FS_IPP);
		FS_DAT.y_right_abs = img_process::get_right_abstract(m_img.yz_img, FS_IPP);
		...
		if (FS_DAT.defect_data.check(ifd_msk)) {
			...
			if (std::abs(yr_off) > 20 || std::abs(xr_off) > 20) {
				log_warning("fiber offes vertical center position too much!");
